...
func createSSHKeysTableUp(db *gorm.DB) error {
	...
	if err := migrator.CreateConstraint(&sshKeyWithFK{}, "Team"); err != nil {
		return err
	}
...
func createSSHKeysTableDown(db *gorm.DB) error {
	return db.Migrator().DropTable(&sshKeyMigration{})
}
