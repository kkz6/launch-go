...
// addRunCommandToComposesUp adds the `run_command` LONGTEXT column the
...
//
// LONGTEXT (rather than VARCHAR) because the user may chain
...
func addRunCommandToComposesUp(db *gorm.DB) error {
	return db.Exec(
		"ALTER TABLE docker_composes ADD COLUMN run_command LONGTEXT NULL",
	).Error
}

func addRunCommandToComposesDown(db *gorm.DB) error {
	return db.Exec(
		"ALTER TABLE docker_composes DROP COLUMN run_command",
	).Error
}
