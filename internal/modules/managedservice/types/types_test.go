...
func TestKind_RunTemplateName(t *testing.T) {
	if KindPostgres.RunTemplateName() != "managedservice/run_postgres.sh" {
		t.Errorf("got %q", KindPostgres.RunTemplateName())
	}
	if KindMySQL.RunTemplateName() != "managedservice/run_mysql.sh" {
		t.Errorf("got %q", KindMySQL.RunTemplateName())
	}
	if KindRedis.RunTemplateName() != "managedservice/run_redis.sh" {
