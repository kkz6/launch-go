...
func Install(opts InstallOptions) *taskrunner.BaseTask {
	...
	script := templates.MustRender("managedservice", opts.Kind.RunTemplateName(), data)
...
func Uninstall(opts UninstallOptions) *taskrunner.BaseTask {
	...
	script := templates.MustRender("managedservice", "managedservice/uninstall.sh", data)
	return taskrunner.NewBaseTask(
...
func Lifecycle(opts LifecycleOptions) *taskrunner.BaseTask {
	...
	script := templates.MustRender("managedservice", "managedservice/lifecycle.sh", data)
	return taskrunner.NewBaseTask(
...
// managed service container. Tail defaults to 100 if unset.
func Logs(opts LogsOptions) *taskrunner.BaseTask {
	...
	script := templates.MustRender("managedservice", "managedservice/logs.sh", data)
	return taskrunner.NewBaseTask(
