...
// JobDeps holds all dependencies for managed-service jobs.
type JobDeps struct {
	*pkgjobs.Deps
...
// BroadcastManagedServiceEvent broadcasts a standard progress event.
func (d *JobDeps) BroadcastManagedServiceEvent(server *servermodels.Server, event, msID, status, message string) {
	if server == nil {
		return
	}
	data := map[string]any{
