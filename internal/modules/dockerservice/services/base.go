...
var (
	...
	ErrAlreadyInstalled = fiberutil.Conflict("This server already has a docker service of this kind")
	// ErrBusy is returned when a lifecycle op runs against a service
	// that is still installing/uninstalling.
	ErrBusy = fiberutil.Conflict("Docker service is busy with another operation")
	// ErrNotRunning is returned by lifecycle ops when the service has
	// no container to act on (failed install, etc.).
	ErrNotRunning = fiberutil.BadRequest("Docker service is not in a runnable state")
)

// ServiceDeps bundles all dependencies the docker-service service needs.
...
// Service holds the business logic for docker services.
type Service struct {
	service.Base
	repos        contracts.RepositoryRegistry
	serverReader contracts.ServerReader
...
// NewService constructs a docker-service service from its dependencies.
