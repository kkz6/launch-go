...
func NewService(repo *Repository, channelFactory *channels.Factory, logger *zerolog.Logger) *Service {
	return &Service{
		repo:           repo,
...
func (s *Service) CreateChannel(ctx context.Context, userID, teamID string, req *CreateChannelRequest) (*NotificationChannel, error) {
	// Parse and validate provider
	provider, err := ParseChannelType(req.Provider)
	if err != nil {
...
func (s *Service) UpdateChannel(ctx context.Context, id, userID, teamID string, req *UpdateChannelRequest) (*NotificationChannel, error) {
...
func (s *Service) DeleteChannel(ctx context.Context, id, teamID string) error {
...
func (s *Service) GetChannel(ctx context.Context, id, teamID string) (*NotificationChannel, error) {
	return s.repo.FindByIDAndTeamID(ctx, id, teamID)
}

// ListChannels lists all notification channels for a team
func (s *Service) ListChannels(ctx context.Context, teamID string) ([]NotificationChannel, error) {
...
func (s *Service) TestChannel(ctx context.Context, id, teamID string, message string) error {
	// Find the channel
	channel, err := s.repo.FindByIDAndTeamID(ctx, id, teamID)
	if err != nil {
		return err
...
func (s *Service) SendToTeam(ctx context.Context, teamID string, notif Notification) error {
...
func (s *Service) SendToChannel(ctx context.Context, channelID string, notif Notification) error {
...
func (s *Service) SetChannelDefault(ctx context.Context, id, teamID string) error {
	channel, err := s.repo.FindByIDAndTeamID(ctx, id, teamID)
	if err != nil {
		return err
	}
...
func (s *Service) DisconnectChannel(ctx context.Context, id, teamID string) error {
	// Verify the channel belongs to the team
...
func (s *Service) ReconnectChannel(ctx context.Context, id, teamID string) error {
	channel, err := s.repo.FindByIDAndTeamID(ctx, id, teamID)
	if err != nil {
		return err
	}
