...
type TeamSubscription struct {
	...
	Status         billingtypes.SubscriptionStatus `json:"status"`
...
func (ts *TeamSubscription) IsSubscribed() bool {
	if ts.Status == billingtypes.SubscriptionStatusActive {
		return true
	}

	if ts.Status == billingtypes.SubscriptionStatusOnTrial && ts.TrialEndsAt != nil {
		return ts.TrialEndsAt.After(time.Now())
	}

	if ts.Status == billingtypes.SubscriptionStatusCancelled && ts.EndsAt != nil {
