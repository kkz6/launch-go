...
type UpdateSSLRequest struct {
	...
	PrivateKey  *string `json:"private_key" validate:"omitempty"`
	Certificate *string `json:"certificate" validate:"omitempty"`
	// max=26 (not len=26) on purpose: the UI sends "" for non-stored TLS
	// settings, and go-playground/validator's omitempty skips only NIL
	// pointers — a non-nil pointer to "" would otherwise trip len=26 and
	// 422 the whole SSL update (e.g. switching to auto/internal/off). A
	// non-empty id is still validated by ssl_service via FindByID, which
	// rejects anything that isn't a real stored-cert row.
	StoredCertificateID *string `json:"stored_certificate_id" validate:"omitempty,max=26"`
}
