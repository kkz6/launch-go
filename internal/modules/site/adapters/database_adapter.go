...
// DatabaseService is the cross-module surface the site module needs.
// The database service exposes both DTO-returning HTTP methods and these
// "Raw" model-returning entrypoints; the site module consumes the latter
// because it needs to read encrypted credentials and other internal
// fields when provisioning a site.
type DatabaseService interface {
	CreateDatabaseRaw(ctx context.Context, serverID, teamID string, req *databasedto.CreateDatabaseRequest, userID *string) (*databasemodels.Database, error)
	GetDatabaseRaw(ctx context.Context, id, serverID, teamID string) (*databasemodels.Database, error)
	GetDatabaseUserRaw(ctx context.Context, id, serverID string) (*databasemodels.DatabaseUser, error)
}

// DatabaseManagerAdapter adapts the database service to the
// site/contracts.DatabaseManager interface.
...
func (a *DatabaseManagerAdapter) CreateDatabase(ctx context.Context, serverID, teamID string, req *databasedto.CreateDatabaseRequest, userID *string) (*databasemodels.Database, error) {
	return a.svc.CreateDatabaseRaw(ctx, serverID, teamID, req, userID)
}
...
func (a *DatabaseManagerAdapter) GetDatabase(ctx context.Context, id, serverID, teamID string) (*databasemodels.Database, error) {
	return a.svc.GetDatabaseRaw(ctx, id, serverID, teamID)
}
...
func (a *DatabaseManagerAdapter) GetDatabaseUser(ctx context.Context, id, serverID string) (*databasemodels.DatabaseUser, error) {
	return a.svc.GetDatabaseUserRaw(ctx, id, serverID)
}
