...
func CheckDaemonStatus() *taskrunner.BaseTask {
	...
	// Supervisor output format: {id}:{id}_00   RUNNING   pid 12345, uptime 2:30:15
	// Where {id} is the ULID queue/daemon ID
	script := `#!/bin/bash
set -euo pipefail
...
supervisorctl status 2>/dev/null | while read -r line; do
    ...
    # Example: 01jvcrnjtmqpxgjasd485tjqym:01jvcrnjtmqpxgjasd485tjqym_00   RUNNING   pid 12345, uptime 2:30:15

    # Extract program name (first field before spaces)
    program_name=$(echo "$line" | awk '{print $1}')

    # Extract daemon ID from program name (format: {id}:{id}_00)
