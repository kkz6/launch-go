...
func (j *InstallCaddyfileJob) Type() string {
	return TypeInstallCaddyfile
}
...
func (j *InstallCaddyfileJob) Handle(ctx context.Context) error {
	...
	if err != nil {
		j.LogError(err, "Failed to install Caddyfile", "site_id", site.ID)
		return err
	}

	exitCode := result.GetExitCode()
	if exitCode != 0 {
		j.LogError(nil, "Caddyfile installation failed", "site_id", site.ID, "exit_code", exitCode)
...
func (j *InstallCaddyfileJob) updateSiteImports(ctx context.Context, server any, site *models.Site) error {
	...
	if !ok {
		...
		_, err = j.RunTaskOnServer(serverModel, task).AsRoot().Dispatch(ctx)
		return err
	}
...
// Type returns the job type
func (j *UpdateCaddyfileJob) Type() string {
	return TypeUpdateCaddyfile
}
...
func (j *UpdateCaddyfileJob) Handle(ctx context.Context) error {
	...
	result, err := j.RunTaskOnServer(server, task).AsRoot().Dispatch(ctx)
	if err != nil {
		j.LogError(err, "Failed to update Caddyfile", "site_id", site.ID)
		return err
	}

	exitCode := result.GetExitCode()
	if exitCode != 0 {
		j.LogError(nil, "Caddyfile update failed", "site_id", site.ID, "exit_code", exitCode)
	...
	j.LogInfo("Caddyfile updated successfully", "site_id", site.ID)
...
func (j *UpdateCaddyfileJob) Failed(ctx context.Context, err error) {
	j.LogError(err, "Update Caddyfile job failed", "site_id", j.Payload.SiteID)
}
...
// Type returns the job type
func (j *UninstallCaddyfileJob) Type() string {
	return TypeUninstallCaddyfile
}
...
`, caddyfilePath)
	...
	result, err := j.RunTaskOnServer(server, task).AsRoot().Dispatch(ctx)
	if err != nil {
		j.LogError(err, "Failed to uninstall Caddyfile", "site_id", site.ID)
		return err
	}

	exitCode := result.GetExitCode()
	if exitCode != 0 {
		j.LogError(nil, "Caddyfile uninstallation failed", "site_id", site.ID, "exit_code", exitCode)
	...
	if err := j.updateSiteImportsAfterRemoval(ctx, site); err != nil {
		j.LogError(err, "Failed to update site imports after removal", "site_id", site.ID)
	}

	j.LogInfo("Caddyfile uninstalled successfully", "site_id", site.ID)
...
func (j *UninstallCaddyfileJob) Failed(ctx context.Context, err error) {
	j.LogError(err, "Uninstall Caddyfile job failed", "site_id", j.Payload.SiteID)
}
...
func (j *UninstallCaddyfileJob) updateSiteImportsAfterRemoval(ctx context.Context, removedSite *models.Site) error {
	...
	// Execute on server
	_, err = j.RunTaskOnServer(server, task).AsRoot().Dispatch(ctx)
	return err
}
