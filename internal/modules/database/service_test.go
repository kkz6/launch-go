...
func TestNewService(t *testing.T) {
	...
	assert.Equal(t, serverRepo, service.serverRepo)
}
...
func TestService_CreateDatabase(t *testing.T) {
	...
	t.Run("returns error for duplicate user name", func(t *testing.T) {
		// Create existing user
		err := repo.CreateUser(ctx, &DatabaseUser{
			ServerID: serverID,
			Name:     "dup_user",
			Password: "password",
		})
...
func TestService_ListDatabases(t *testing.T) {
	...
	err := repo.Create(ctx, &Database{ServerID: serverID, Name: "db1"})
	require.NoError(t, err)
	err = repo.Create(ctx, &Database{ServerID: serverID, Name: "db2"})
	require.NoError(t, err)
...
func TestService_ListDatabaseUsers(t *testing.T) {
	...
	// Create some users
	err := repo.CreateUser(ctx, &DatabaseUser{ServerID: serverID, Name: "user1", Password: "pass"})
	require.NoError(t, err)
	err = repo.CreateUser(ctx, &DatabaseUser{ServerID: serverID, Name: "user2", Password: "pass"})
	require.NoError(t, err)
...
func TestService_UpdateDatabaseUser(t *testing.T) {
	...
	t.Run("updates user databases", func(t *testing.T) {
		// Create databases
		db1 := &Database{ServerID: serverID, Name: "upd_db1"}
		err := repo.Create(ctx, db1)
		require.NoError(t, err)

		db2 := &Database{ServerID: serverID, Name: "upd_db2"}
...
func TestService_UpdateDatabaseUser_EmptyDatabases(t *testing.T) {
	...
	database := &Database{ServerID: serverID, Name: "db_upd_empty"}
	err := repo.Create(ctx, database)
	require.NoError(t, err)
...
func TestService_DispatchFunctions_WithNilQueue(t *testing.T) {
	...
	database := &Database{ServerID: serverID, Name: "dispatch_test_db"}
	err := repo.Create(ctx, database)
	require.NoError(t, err)
