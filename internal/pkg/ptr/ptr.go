...
// Example:
//
//	response.Count = ptr.NilIfZero(count) // nil if count == 0
...
// Example:
//
//	response.Description = ptr.NilIfEmpty(desc)
...
// Map applies a function to the dereferenced value if non-nil.
//
// Example:
//
//	upper := ptr.Map(name, strings.ToUpper) // nil if name is nil
...
// MapOr applies a function if non-nil, otherwise returns default.
//
// Example:
//
//	length := ptr.MapOr(name, len, 0)
func MapOr[T, R any](p *T, fn func(T) R, defaultVal R) R {
	if p == nil {
		return defaultVal
	}
	return fn(*p)
