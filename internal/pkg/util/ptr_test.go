...
func TestPtrMap(t *testing.T) {
	t.Run("applies function when non-nil", func(t *testing.T) {
		s := "hello"
		result := PtrMap(&s, strings.ToUpper)
		assert.NotNil(t, result)
		assert.Equal(t, "HELLO", *result)
	})
	...
	t.Run("works with type conversion", func(t *testing.T) {
		s := "hello"
		result := PtrMap(&s, func(v string) int { return len(v) })
		assert.NotNil(t, result)
		assert.Equal(t, 5, *result)
	})
}

func TestPtrMapOr(t *testing.T) {
	t.Run("applies function when non-nil", func(t *testing.T) {
		s := "hello"
		assert.Equal(t, 5, PtrMapOr(&s, func(v string) int { return len(v) }, 0))
	})

	t.Run("returns default when nil", func(t *testing.T) {
		var s *string
		assert.Equal(t, -1, PtrMapOr(s, func(v string) int { return len(v) }, -1))
	})
}
